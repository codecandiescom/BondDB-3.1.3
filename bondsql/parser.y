...
%token L_IDENT
%token L_EQ L_IS L_LIKE L_ILIKE L_GT L_LT L_GEQ L_LEQ L_DIFF L_REGEXP L_REGEXP_CI L_NOTREGEXP L_NOTREGEXP_CI L_SIMILAR
...
simple_table: L_IDENT                       {$$ = sql_table_build ($1); mem_free ($1);}
        ;
...
dotted_name: L_IDENT                    {$$ = g_list_append (NULL, mem_strdup ($1)); mem_free ($1);}
        | L_IDENT L_DOT dotted_name     {$$ = g_list_prepend ($3, mem_strdup ($1)); mem_free ($1);}
	| L_IDENT L_DOT L_TIMES		{$$ = g_list_append (NULL, mem_strdup ($1)); mem_free ($1);
					 $$ = g_list_append ($$, mem_strdup ("*"));}
        ;

field_name: dotted_name                 {$$ = $1;}
        | L_TIMES                       {$$ = g_list_append (NULL, mem_strdup ("*"));}
        | L_NULL                        {$$ = g_list_append (NULL, mem_strdup ("null"));}
        | L_STRING                      {$$ = g_list_append (NULL, $1);}
	| L_UNSPECVAL                   {$$ = g_list_append (NULL, mem_strdup (""));}
        ;
