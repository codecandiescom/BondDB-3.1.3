...
static gint
db_buildfieldprimarykey(DbTableDef *db)
	{
	DbRecordSet *res;
	gchar *query, *value;
	gchar **arr, **ptr;
	...
	query = mem_strdup_printf("SELECT i.indkey, i.indisprimary, i.indisunique "
	...
	if (db_dbcheckresult(res) != 0)
		{
		debugmsg("failed to find any primary keys. Make sure you have postgresql 7.3 or higher installed.");
		db_dbclear(res);
		mem_free(query);
		return -1;
		}
	...
	num = db_dbnumrows(res);
	for (i=0;i<num;i++)
		{
		value = PQgetvalue (res->res, i, 0);
		if (!value)
			continue;
		...
		value = PQgetvalue (res->res, i, 1);
		if (value && *value=='t')
			primary = TRUE;
		value = PQgetvalue (res->res, i, 2);
		if (value && *value=='t')
			unique = TRUE;
		...
		while (*ptr!=NULL)
			{
			columnn = atoi(*ptr);
			if (columnn > 0 && columnn-1 < db->num)
				{
				/* cause i start at 0 and pg_indx starts at 1 */
				columnn--;
